...
while True:
    ...
    lmList, bbox = detector1.findPosition(img, draw=True)
    if len(lmList) != 0:
        #Fiter based on size
        ...
        if 250<area<650:
            ...
            # Find Disttance between Nose and schoulders
            ...
            vol1 = np.interp(length1, [130, 190], [MinVol1, MaxVol1])
            #print(int(length1), vol1) # Linke Seite
            vol1Bar = np.interp(length1, [130, 190], [400, 150])
            vol1Per = np.interp(length1, [130, 190], [0, 100])

            vol2 = np.interp(length2, [360, 500], [MinVol2, MaxVol2])
            print(int(length2), vol2) # Rechte seite
            vol2Bar = np.interp(length2, [370, 490], [400, 150])
            vol2Per = np.interp(length2, [370, 490], [0, 100])
