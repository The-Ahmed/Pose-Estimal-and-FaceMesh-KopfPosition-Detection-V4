    ...
    if results.multi_face_landmarks is not None:
        ...
        if time.time() - timeStart < totalTime:
            ...
            #cvzone.putTextRect(image, 'Time: 20', (450, 25), scale=2, offset=4)  # Timer
                cvzone.putTextRect(image, f'Time: {int(totalTime - (time.time() - timeStart))}',
                                (450, 25), scale=2, offset=4)  # Timer
        else:

            cvzone.putTextRect(image, 'Muedichkeit', (350, 25), scale=2, offset=4)
            cvzone.putTextRect(image, 'Press R to restart', (325, 55), scale=2, offset=4)
